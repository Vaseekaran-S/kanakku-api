...
const { getUser, updateUser } = require("./users.service");
...
const emailTokenVerification = async (token) => {
    ...
    if (isValid) {
        await UserModel.updateOne({ email: isValid?.userMail }, { isEmailVerified: true, emailVerificationToken: "" })
...
const sendForgotPasswordToken = async (email) => {
    ...
    await updateUser(email, { resetPasswordToken: token })
    const data = {
        email, token, name
    }
...
const resetUserPassword = async ({ newPassword, token }) => {
    try {
        const { userMail } = await verifyJwtToken(token);
        if (!userMail) return { type: "error", message: "Invalid token!" };

        const user = await UserModel.findOne({ email: userMail, resetPasswordToken: token }, 'password');
        if (!user) return { type: "error", message: "Token expired!" };

        const isSamePassword = await verifyHashPassword(newPassword, user.password);
        if (isSamePassword) {
            return { type: "warning", message: "Try a different password!" };
        }

        const hashedPassword = await hashPassword(newPassword);
        await UserModel.updateOne({ email: userMail }, { password: hashedPassword, resetPasswordToken: "" });
